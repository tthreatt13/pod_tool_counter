...
import { Plus, Search, TrendingUp, Mic, Info, Loader2, Link2, X, Sparkles, Youtube, CheckCircle2 } from 'lucide-react';
      ...
      <section>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          {sortedTools.slice(0, 5).map((tool, index) => (
            <LeaderboardRow key={tool.id} tool={tool} rank={index + 1} />
          ))}
        </div>
      </section>
...
const AddEpisodeModal = ({ onComplete }: { onComplete: (res: ExtractionResult[]) => void }) => {
  ...
  const handleProcess = async () => {
    ...
    if (allResults.length > 0) {
      ...
      setLoading(false);
      setError('None of the URLs could be processed correctly.');
    }
  };
                ...
                {tools.length === 0 ? (
                  ...
                  <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    {[...tools].sort((a,b) => b.mentionCount - a.mentionCount).map((tool, i) => (
                      <LeaderboardRow key={tool.id} tool={tool} rank={i + 1} />
                    ))}
                  </div>
                )}
              </div>
